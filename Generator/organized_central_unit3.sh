...
new_folder_path1="$parent_dir/locust/november_all/nantes/hyperthreading/$category/$date_str"
new_folder_path_backup="$parent_dir/locust/november_all/nantes/hyperthreading/$category/$date_str/backup"

istio_path="../istio_metrics.json"
metric_path="../teastore_grenoble3.json"

workload_date=$(date +"%Y-%m-%d")
#workload_dir="../Load/profiles_$workload_date"
workload_dir="../Load/profiles_2024-07-31"

host="http://econome-4.nantes.grid5000.fr:30080/tools.descartes.teastore.webui"

workload_files=($(ls "$workload_dir"/*.csv))

export KUBECONFIG=~/admin_load3.conf
  ...
  env INTENSITY_FILE=$warm locust -f /home/erods-chouette/Documents/synchronizeExperimentation/workload_generators/locust/teastore_locustfile-custom-scale2.py --headless --host $host
  ...
  env INTENSITY_FILE=$file_name locust -f /home/erods-chouette/Documents/synchronizeExperimentation/workload_generators/locust/teastore_locustfile-custom-scale2.py --headless --csv $log_exp_folder_path --host $host
