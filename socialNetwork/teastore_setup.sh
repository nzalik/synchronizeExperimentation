...
kubectl create -f $dir/conf/teastore/teastore-ribbon-with-affinity.yaml --namespace default
kubectl rollout status deployment teastore-webui -n default

host_name=$(kubectl get pod --selector 'run=teastore-webui' -n default -o jsonpath='{.items[*].spec.nodeName}')
host_ip=$(kubectl get node $host_name -o jsonpath='{.status.addresses[0].address}')
