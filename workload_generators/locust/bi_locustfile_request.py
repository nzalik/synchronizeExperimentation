...
from helpers import random_string_generator, random_date_generator, postfix
import locust
import numpy as np

from locust import events
from locust import task, constant, HttpUser,LoadTestShape
...
class Requests:
    ...
    def __init__(self, client):
        ...
        print("on choisirt un utilisateur")
        print(user)
        self.bearer = ""
        self.user_id=""
        self.user_name = user
    ...
    def login(self, expected):
        ...
        if response is not None and response.status_code == 200:
            ...
            response_as_json_all = response.json()
            response_as_json = response_as_json_all.get("data")

            if response_as_json is not None and  response.json()["status"] == 1 :
    ...
    def select_contact(self, expected):
        print("######################################################################")
        print("on est dans la fonction selct contact")
        print("#####################################################################")
        head = {"Accept": "application/json",
                "Content-Type": "application/json", "Authorization": self.bearer}
        req_label = sys._getframe().f_code.co_name + postfix(expected)
        ...
        if response_contacts is not None and response_contacts.status_code == 200:
